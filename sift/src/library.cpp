...
float lpnorm(float *u,int fpow,int size)
{
	...
	if (fpow<0){ printf("lpdist_mask: pow must take positive values\n");  return -1.f; }
	...
	{		
			...
			if (fpow==0) norm = MAX(dif,norm);
			else if (fpow==1) norm += dif;
			else if (fpow==2) norm += dif*dif;
			else	norm += powf(dif, (float) fpow);
...
float lpdist(float *u,float *v,float *mask,int fpow,int size)    
{

	if (fpow<0){ printf("lpdist_mask: pow must take positive values\n");  return -1.f; }

	float sdif=0.0;	float pused = 0.0;
	for(int i=0; i<size ; i++)
		if (!mask || (mask && mask[i] > 0.0)) {
		
		  float  dif = v[i] - u[i];   dif = (float) fabsf(dif);

		  if (fpow==0) sdif = MAX(dif,sdif);
		  else if (fpow==1) sdif+=dif;
		  else if (fpow==2) sdif += dif*dif;
		  else	sdif += powf(dif, (float) fpow);
...
void addnoise_var_afine(float *u,float *v,float a,float b,long int /*randinit*/, int size)
{
	...
	for (int i=0; i< size; i++) {
		...
		double a0 = rand()/(double)RAND_MAX;
		double b0 = rand()/(double)RAND_MAX;
...
void compute_gradient_orientation(float* igray,float *grad, int width, int height)
{
    ...
    for (r = 0; r < rows; r++)
      for (c = 0; c < cols; c++) {
        if (c == 0)
          xgrad = 2.0f * (igray[r*cols+c+1] - igray[r*cols+c]);
        else if (c == cols-1)
          xgrad = 2.0f * (igray[r*cols+c] - igray[r*cols+c-1]);
        else
          xgrad = igray[r*cols+c+1] - igray[r*cols+c-1];
        if (r == 0)
          ygrad = 2.0f * (igray[r*cols+c] - igray[(r+1)*cols+c]);
        else if (r == rows-1)
          ygrad = 2.0f * (igray[(r-1)*cols+c] - igray[r*cols+c]);
        else
...
{
	for(int i=width*height-1; i>0; i--)	out[i] = (red[i] + green[i] + blue[i]) /3.0f;
}
